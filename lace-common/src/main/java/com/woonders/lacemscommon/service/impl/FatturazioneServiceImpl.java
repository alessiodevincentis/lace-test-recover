...
public class FatturazioneServiceImpl implements FatturazioneService {
    ...
    @Override
    public ViewModelPage<ClientePratica> searchPraticheDaPerfezionare(final Date dataInizio, final Date dataFine,
        ...
        try {
            findPraticheDaPerfezionareExpression = predicateHelper
                    .getPredicateForFindPraticheDaPerfezionare(dataInizio, dataFine, liquidazione, liquidata, quietanzata,
    ...
    @Override
    public BigDecimal sumTotaleMontantePraticheDaPerfezionare(final Date dataInizio, final Date dataFine, final boolean liquidata,
    ...
    @Override
    public BigDecimal sumTotaleProvvigionePraticheDaPerfezionare(final Date dataInizio, final Date dataFine, final boolean liquidata,
                                                                 final boolean liquidazione, final boolean quietanzata, final boolean perfezionamentoSelected, final Long currentAziendaId, final Role.RoleName fatturazioneWriteRoleName) {
        BooleanExpression findPraticheDaPerfezionareExpression = null;
        try {
            findPraticheDaPerfezionareExpression = predicateHelper
    ...
    @Override
    public ViewModelPage<ClientePratica> searchPratichePerfezionate(final Date dataInizio, final Date dataFine,
        ...
        try {
            pratichePerfezionateExpression = predicateHelper.getPredicateForFindPratichePerfezionate(
                    dataInizio, dataFine, operatorId, aziendaId, operatorToSearchId,
    ...
    @Override
    public BigDecimal sumTotaleMontantePratichePerfezionate(final Date dataInizio, final Date dataFine, final String username, final Long currentAziendaId, final long operatorId, final long aziendaId, final Role.RoleName fatturazioneReadRoleName) {
        Long operatorToSearchId = null;
        if (!StringUtils.isEmpty(username)) {
            operatorToSearchId = operatorService.findByUsernameViewModel(username).getId();
        }
    ...
    @Override
    public BigDecimal sumTotaleProvvigionePratichePerfezionate(final Date dataInizio, final Date dataFine, final String username, final Long currentAziendaId, final long operatorId, final long aziendaId, final Role.RoleName fatturazioneReadRoleName) {
        Long operatorToSearchId = null;
        if (!StringUtils.isEmpty(username)) {
            operatorToSearchId = operatorService.findByUsernameViewModel(username).getId();
        }
    ...
    @Override
    @Transactional
    public void perfezionaPratiche(final Date dataInizio, final Date dataFine, final boolean liquidata,
                                   final boolean liquidazione, final boolean quietanzata, final boolean perfezionamentoSelected, final Long currentAziendaId, final Role.RoleName fatturazioneWriteRoleName) {
        final BooleanExpression findPraticheDaPerfezionareExpression;
        try {
            findPraticheDaPerfezionareExpression = predicateHelper
