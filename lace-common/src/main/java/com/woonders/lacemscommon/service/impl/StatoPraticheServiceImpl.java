...
public class StatoPraticheServiceImpl extends AbstractAppServiceImpl implements StatoPraticheService {
	...
	@Override
	public long countByStatoPraticaAndTipoClienteAndUsernameAndOperators(Long currentAziendaId, long currentOperatorId,
			RoleName clientiReadRoleName, String statoPratica, String tipoPratica, List<String> operators) {
		
		try {
            return praticaRepository.count(predicateHelper.getPredicateForfindByStatoPraticaAndTipoClienteAndUsernameAndOperators(
                    currentAziendaId, currentOperatorId, clientiReadRoleName, statoPratica, tipoPratica, operators));
        } catch (final PermissionDeniedException e) {
            return 0;
        }
	}
	...
	@Override
	public long countEstinzioniByStatoConteggioEstinzioneAndTipoClienteAndUsernameAndOperators(Long currentAziendaId,
			long currentOperatorId, RoleName clientiReadRoleName, StatoConteggioEstinzione statoConteggioEstinzione,
			String tipoEstinzione, List<String> operators) {
		
		final Date notifyDate = calcNotifyDateForFindEstinzioni(currentAziendaId);

        try {
            final BooleanExpression statoConteggioEstinzioneExpression = predicateHelper.
            		getPredicateForFindEstinzioniByStatoConteggioEstinzioneAndTipoClienteAndUsernameAndOperators(currentAziendaId, currentOperatorId,
                            clientiReadRoleName, statoConteggioEstinzione, notifyDate, tipoEstinzione, operators);
            return estinzioneRepository.count(statoConteggioEstinzioneExpression);
        } catch (final PermissionDeniedException e) {
            return 0;
        }
